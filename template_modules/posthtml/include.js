...
/**
 * Обработка атрибутов узла с заменой алиасов.
 * @param {Object} attrs - Объект атрибутов
 * @param {boolean} prependDot - Добавлять ли точку перед значением
 * @returns {string|false} - Найденное значение src/url или false
 */
...
/**
 * Обработка тегов и замена алиасов в атрибутах.
...
export default (options = {}) => {
	...
	return function posthtmlInclude(tree) {
		...
		tree.match({ attrs: true }, (node) => {
			...
			// Для стандартных HTML-тегов prependDot = false, для других true
			...
			if (node.tag === 'include' && src) {
				...
				// Подключение Tailwind CSS или Reset CSS
