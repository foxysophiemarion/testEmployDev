...
class SelectConstructor {
	constructor(props, data = null) {
		...
		this.selectClasses = {
			...
			classSelectTitle: "select__title--icon-spoller", // Заголовок
	...
	selectСlose(selectItem) {
		...
		if (!selectOptions.classList.contains('_slide')) {
			...
			selectItem.classList.remove('open-animated'); // Удаляем анимационный класс при закрытии
	...
	selectAction(selectItem) {
		...
		setTimeout(() => {
			if (!selectOptions.classList.contains('--slide')) {
				...
				if (willOpen) {
					selectItem.classList.add('open-animated'); // Добавляем анимационный класс при открытии
				} else {
